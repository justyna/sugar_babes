...
public class Application {
  ...
  public String index(@RequestBody ArenaUpdate arenaUpdate) {
   ...
   /* if (arenaUpdate.arena.state.isEmpty()) {
        return "F";
    }*/
   ...
   */
   /* Integer x = arenaUpdate.arena.dims.get(0);
    Integer y = arenaUpdate.arena.dims.get(1);

   for (String key: arenaUpdate.arena.state.keySet()) {
    PlayerState playerState = arenaUpdate.arena.state.get(key);
    if (playerState.x + 1 == x || playerState.x - 1 == x ) {
        return "L";
    }
    if (playerState.y + 1 == )
   }*/
